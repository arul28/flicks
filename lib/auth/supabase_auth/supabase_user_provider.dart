...
class FlicksSupabaseUser extends BaseAuthUser {
  FlicksSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> flicksSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FlicksSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
